...
public class Graph<V,E> implements GraphInterface<V,E> {
    ...
    private void colouring() {
        ...
        boolean disponivel[] = new boolean[numVert];
        ...
        for(V vertice : vertices.keySet()){
            Vertex verticeAux = (Vertex) vertice;
            Iterable<Vertex> vAdjs = (Iterable<Vertex>) adjVertices(vertice);
                 //verifica as cores dos vertices adjacentes e mete-as a falso
            for (Vertex vertex : vAdjs) {
                int j = vertex.getKey();
                if (resultado[j] != -1) {
                    disponivel[j] = false;
                }
                ...
                int cor;
                for (cor = 0; cor < numVert; cor++) {
                    if (disponivel[cor]) {
                        break;
                    }
                }
                //preenche o vertex com a tal cor
                //g.getVertex(i).setCor(cor);
                resultado[verticeAux.getKey()] = cor;
            ...
            */
            ...
            for (int k = 0; k < numVert; k++) {
                System.out.println("País " + k + " tem a cor:  "
                        + resultado[k]);
